    ...
    async function fetchSupplies(page, size) {
        ...
        try {
            const response = await axios.get('http://localhost:8080/api/supplies', { params });
    ...
    function populateSupplyTable(supplies) {
        ...
        supplies.forEach(supply => {
            ...
            tr.innerHTML = `
                <td>${supply.id}</td>
                <td>${supply.name}</td>
                <td>${supply.address.address} (${supply.address.phone})</td>
                <td>${supply.address.phone}</td>
    ...
    async function deleteSupply(supplyId) {
        if (confirm("Bạn có chắc chắn là muốn xoá nhà cung cấp này không?")) {
            try {
                const response = await axios.delete(`http://localhost:8080/api/supplies/${supplyId}`);
